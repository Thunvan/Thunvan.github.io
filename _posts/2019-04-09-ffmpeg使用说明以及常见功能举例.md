...
* 命令：把2个视频左右合并为1个，并在中间设置一条均匀来回滑动的线
  ...
  ```bash
  ./ffmpeg -i ./section_20.mp4 -i ./section_20.mp4 -filter_complex "[0:v] setpts=PTS-STARTPTS, scale=w=1920:h=1080:force_original_aspect_ratio=decrease:flags=lanczos [left];[1:v] setpts=PTS-STARTPTS, scale=w=1920:h=1080:force_original_aspect_ratio=decrease:flags=lanczos,curves=preset=color_negative [right];[left][right] blend=shortest=1:all_expr='if(lte(X,W*abs((mod(T/10\,2)-1))),A,B)' [scene]; color=c=black:size=2x1080 [line];[scene][line] overlay=shortest=1:format=yuv420:x=W*abs((mod(t/10\,2)-1)) [out]" -map "[out]" -map "0:a" -movflags faststart -vcodec libx264 -x264opts crf=16 -y merge2.mp4
  ```
